...
// Variáveis padrão em branco (para um novo cadastro)
$id = '';
$nome = '';
$preco = '';
$descricao = '';
...
// Se receber um ID pela URL, significa que vamos EDITAR um produto existente
...
// Se o formulário for enviado (botão Salvar clicado)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = $_POST['id'] ?? '';
    $nome = $_POST['nome'] ?? '';
    $preco = str_replace(',', '.', $_POST['preco'] ?? '0'); 
    $descricao = $_POST['descricao'] ?? '';
    $categoria_id = $_POST['categoria_id'] ?? ''; // Captura a categoria selecionada
    ...
    // 2. SALVAR NO BANCO DE DADOS INCLUINDO A CATEGORIA_ID
    if (empty($id)) {
